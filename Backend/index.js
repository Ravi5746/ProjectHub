...
if (isProduction) {
    console.log(`[CORS] Allowed Origins: ${[...new Set(allowedOrigins)].join(', ')}`)
}
...
app.use(cors({
    origin: (origin, callback) => {
        ...
        const isAllowed = allowedOrigins.some(allowed => {
            // Match exact or match without protocol if allowed origin has no protocol
            return allowed === origin || allowed.replace(/^https?:\/\//, '') === origin.replace(/^https?:\/\//, '');
        });
